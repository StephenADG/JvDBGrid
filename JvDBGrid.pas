...
// $Id: JvDBGrid.pas 13499 2013-02-04 09:07:28Z ahuser $
...
{*** JvDBUltimGrid ***}
	...
	TIndexNotFoundEvent = procedure(Sender: TJvDBGrid; FieldsToSort: TSortFields;
		IndexFieldNames: string; DescFields: string; var Retry: Boolean) of object;
	TUserSortEvent = procedure(Sender: TJvDBGrid; var FieldsToSort: TSortFields;
		SortString: string; var SortOK: Boolean) of object;
	TRestoreGridPosEvent = procedure(Sender: TJvDBGrid; SavedBookmark: TBookmark;
		SavedRowPos: Integer) of object;
	TCheckIfValidSortFieldEvent = function(Sender: TJvDBGrid;
		FieldToSort: TField): Boolean of object;
	TGetSortFieldNameEvent = procedure(Sender: TJvDBGrid; var FieldName: string) of object;
...
{*** JvDBUltimGrid ***}
  ...
  private
    ...
    FOnBeforePaint: TNotifyEvent;
		...
		{ if false it behaves like TJvDBUltimGrid, if true it behaves like the old TJvDBGrid }
		FOldGridBehavior: Boolean; { My code }

		{ Highlight colors }
		FHighlightColor: TColor;
		FHighlightFontColor: TColor;
		{ MultiSelect colors }
		FMultiSelectColor: TColor;
		FMultiSelectFontColor: TColor;
    ...
    {$ENDIF ~COMPILER14_UP}
		...
		function GetColumnMaxWidth(const CurrentColWidth: Integer): Integer; { My code }
		function FirstLineOfText(const S: string): string; {$IFDEF SUPPORTS_INLINE} inline; {$ENDIF}	{ My code }
  ...
  protected
    ...
    procedure ShowSelectColumnClick; dynamic;
		...
		procedure DoTitleClick_1(ACol: Longint; AField: TField);
		procedure DoTitleClick_2(ACol: Longint; AField: TField);
    ...
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
		...
		procedure DrawCellHighlight(const ARect: TRect;
			AState: TGridDrawState; ACol, ARow: Integer); reintroduce; virtual;
  ...
  public
    ...
    destructor Destroy; override;
		...
		procedure DefaultDrawColumnCell(const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState); reintroduce;
    ...
    procedure EndUpdate;
		...
		function ColumnByFieldName(const FldName: string): TColumn; { My code }
		...
		{*** JvDBUltimGrid ***}
{
		...
		{ Set behaviour of the JvDBGrid. }
		property OldGridBehavior: Boolean read FOldGridBehavior write FOldGridBehavior default False;
		...
		{ MultiSelect colors }
		procedure SetMultiSelectColor(const Value: TColor);
		procedure SetMultiSelectFontColor(const Value: TColor);
		property MultiSelectColor: TColor read FMultiSelectColor write SetMultiSelectColor default clHighlight;
		property MultiSelectFontColor: TColor read FMultiSelectFontColor write SetMultiSelectFontColor default clHighlightText;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: http://jvcl.svn.sourceforge.net/svnroot/jvcl/trunk/jvcl/run/JvDBGrid.pas $';
    Revision: '$Revision: 13499 $';
    Date: '$Date: 2013-02-04 10:07:28 +0100 (Mon, 04 Feb 2013) $';
    LogPath: 'JVCL\run'
  );
{$ENDIF UNITVERSIONING}
...
begin
  ...
  inherited DefaultDrawing := False;
	...
	// MultiSelect colors
	FMultiSelectColor := clHighlight;
	FMultiSelectFontColor := clHighlightText;
...
begin
  if MultiSelect and Assigned(DataLink) and DataLink.Active then
...
begin
	...
	{ My code }
	with TInternalInplaceEdit(Result) do begin
		BevelInner := bvNone;
		BevelKind := bkNone;
		BevelOuter := bvNone;
...
begin
  ...
  begin
    SetFocus;
		Result := Focused or (Assigned(InplaceEditor) and InplaceEditor.Focused) or
												 (Assigned(FCurrentControl) and FCurrentControl.Focused);
  end;
end;
...
begin
	...
	begin
		if Enabled { My code } and Odd(FCurrentDrawRow + FixedRows) then
		begin
	...
	if Highlight then
	begin
		AFont.Color := FHighlightFontColor;
		Background := FHighlightColor;
	end;
...
begin
	if FOldGridBehavior then
		DoTitleClick_1(ACol, AField)
	else
		DoTitleClick_2(ACol, AField);
end;

procedure TJvDBGrid.DoTitleClick_1(ACol: Longint; AField: TField);
...
procedure TJvDBGrid.DoTitleClick_2(ACol: Longint; AField: TField);
var
	Keys: TKeyboardState;
...
begin
  ...
  begin
    ...
    begin
			...
			if Assigned(ColumnAtDepth(Columns[ACol], ARow)) then
...
begin
  ...
  try
    ...
    begin
      ...
      if FTitleButtons and Assigned(DataLink) and DataLink.Active and
      ...
      begin
        ...
        begin
          ...
          if WasAlwaysShowEditor and (Button = mbRight) {and Assigned(PopupMenu)} then
			...
			end;
{
			...
			if Assigned(OnMouseDown) and not InheritedCalled then
...
begin
  Cell := MouseCoord(X, Y);
  if FTracking and Assigned(FPressedCol) then
  begin
    ...
    begin
      ...
      if Assigned(DataLink) and DataLink.Active and (ACol >= 0) and
...
{ My code }
function TJvDBGrid.FirstLineOfText(const S: string): string;
var
	C: Char;
begin
	Result := '';
...
var
	...
	procedure DoDefaultDrawColumnCell;
	begin
		if HighlightCell(DataCol, DataRow, Value, State) then
			DrawCellHighlight(Rect, State, DataCol, DataRow);
		if not Enabled then
			Canvas.Font.Color := clGrayText;
		WriteText(Canvas, Rect, 3, 2, FirstLineOfText(Value), Column.Alignment,
...
begin
	...
	if Assigned(Column.Field) then begin
		...
		if FShowMemos and IsMemoField(Column.Field) and not Assigned(Column.Field.OnGetText) then
			Value := Column.Field.AsString;

		DoDefaultDrawColumnCell;
	end
...
begin
  if dgIndicator in Options then
    Dec(ACol, IndicatorOffset);
  if Assigned(DataLink) and DataLink.Active and (ACol >= 0) and (ACol < Columns.Count) then
  begin
    Result := Columns[ACol];
...
var
  ...
  DrawColumn: TColumn;
	...
	CanDrawEdge: Boolean; { My code }
//  OSOffset: Integer; //Bug in Windows 7
...
begin
  ...
  if (dgIndicator in Options) and (ACol = 0) and (ARow - TitleOffset >= 0) and
    MultiSelect and Assigned(DataLink) and DataLink.Active and
    ...
    begin
      if ARow - TitleOffset <> DataLink.ActiveRecord then
        Indicator := 0
			...
			FMsIndicators.BkColor := FixedColor; // if comment this line, default indicator draws
...
(*
				{ DrawEdge bugs:
					- not draw when UseXPThemes-runtime,
					- draw when gdsGradient-csDesigning }
...
*)
				...
				if (csDesigning in ComponentState) then
					if (DrawingStyle = gdsGradient)
					{$IFDEF JVCLThemesEnabled}
					or (UseXPThemes and StyleServices.Enabled)
					{$ENDIF JVCLThemesEnabled}
				then
					CanDrawEdge:=False;
      ...
      if FTitleButtons and Assigned(AField) then
      begin
...
//            DrawCellBackground(Rect(TextRect.Right, TitleRect.Top, TitleRect.Right, TitleRect.Bottom), FixedColor, AState, ACol, ARow - TitleOffset);
	...
	{ My code - Fix cell-edge display problem for fixed columns when window is maximised }
	if (gdFixed in AState) and not (DrawingStyle = gdsClassic) and (ARow >= TitleOffset) and (ACol <= FixedCols)
		and ((ACol > 0) and (dgIndicator in Options) or (ACol >= 0) and not(dgIndicator in Options)) then
	begin
		{ These edge styles are just a sample }
		if (DrawingStyle = gdsGradient) then DrawEdge(Canvas.Handle, ARect, EDGE_ETCHED, BF_RECT)
		{$IFDEF JVCLThemesEnabled}
		else
		if UseXPThemes and StyleServices.Enabled then
...
begin
	...
	if not Highlight and (ReadOnlyCellColor <> clDefault) and
		Assigned(Field) and (not Field.CanModify or not CanEditCell(Field)) then
	begin
		...
		begin
			...
			if Field.Lookup and Assigned(Field.LookupDataSet) and (Field.LookupResultField <> '')
	...
	begin
		if HighlightCell(DataCol, Column.Index, '', State) then
			DrawCellHighlight(Rect, State, DataCol, Column.Index);
	...
	{ My code }
	if DefaultDrawing and Highlight
...
begin
	...
	{ My code }
	FCanResizeColumn := (State = gsColSizing) and (FColumnResize <> gcrNone); //  If true, mouse double clicking can resize column.
...
procedure TJvDBGrid.SetHighlightColor(const Value: TColor);
begin
...
procedure TJvDBGrid.SetHighlightFontColor(const Value: TColor);
begin
...
procedure TJvDBGrid.SetMultiSelectColor(const Value: TColor);
begin
	if FMultiSelectColor <> Value then
	begin
		FMultiSelectColor := Value;
		Invalidate;
	end;
end;

procedure TJvDBGrid.SetMultiSelectFontColor(const Value: TColor);
begin
	if FMultiSelectFontColor <> Value then
	begin
		FMultiSelectFontColor := Value;
		Invalidate;
	end;
end;
...
function TJvDBGrid.GetMaxDisplayText: string;
	...
	var
		S: string;
	begin
		if IsMemoField(Columns[FResizeColumnIndex].Field) then begin
			if FShowMemos then
				S := Columns[FResizeColumnIndex].Field.AsString
			else
			if FShowGlyphs then
				S := 'A' // Any chatacter will do.
			else
				S := Columns[FResizeColumnIndex].Field.DisplayText; // Default value '(MEMO)'
		end else
			S := Columns[FResizeColumnIndex].Field.DisplayText;

		{	Some databases (e.g. PostgreSQL -> text field; MSSQL Server -> varchar(max) field) return ftWideString datatype for text fields,
			so we need to test any character field if is used as text field. }
		S := FirstLineOfText(S);
...
function TJvDBGrid.GetColumnMaxWidth(const CurrentColWidth: Integer): Integer;
const
...
var
  ...
  TempDC: HDC;
	...
	{ My code }
	procedure ToggleColWidth(AWidth: Integer);
	begin
		if AWidth = DefaultColWidth then
			Result := 12
		else
...
begin
  if Assigned(Columns[FResizeColumnIndex].Field) then
  begin
    ...
    begin
      ...
      try
        ...
        GetTextMetrics(Canvas.Handle, TM);
				...
				if (Result < DefaultColWidth) and (CurrentColWidth <> DefaultColWidth) then { My code }
          Result := DefaultColWidth;
      finally
        if RestoreCanvas then
        begin
          ...
          ReleaseDC(0, TempDc);
			...
			ToggleColWidth(CurrentColWidth); { My code }
	end
	else
...
//		if FColumnResize <> gcrNone then
			Columns[FResizeColumnIndex].Width := GetColumnMaxWidth(Columns[FResizeColumnIndex].Width); { My code }
	end
...
begin
  ...
  if Self.Visible and Control.Visible and Assigned(Self.Parent) and
     Self.Parent.Visible and Assigned(ParentForm) and ParentForm.Visible then
  begin
...
begin
  ...
  begin
    ...
    if Assigned(FCurrentControl) and FCurrentControl.Focused then
      Abort;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      CurrentEditor := FControls.ControlByName(FCurrentControl.Name);
      if Assigned(CurrentEditor) and CurrentEditor.LeaveOnUpDownKey and
...
begin
  ...
  if (Row <> LastRow) and Assigned(DataLink) and DataLink.Active then
...
{ My code }
function TJvDBGrid.ColumnByFieldName(const FldName: string): TColumn;
var
	I: Integer;
begin
	Result := nil;
	for I := 0 to Columns.Count - 1 do
		if AnsiSameText(Columns[I].FieldName, FldName) then
		begin
			Result := Columns[I];
			Break;
		end;
...
{*** JvDBUltimGrid ***}
procedure TJvDBGrid.Sort(FieldsToSort: TSortFields);
const
  cIndexDefs = 'IndexDefs';
  cIndexName = 'IndexName';
...
//		NewIndexName := cIndexPrefix + IntToStr(IndexDefs.Count);
...
begin
  ...
  case SortWith of
    swIndex:
      if not InheritsFromByName(DataSource.DataSet.ClassType, 'TTable') then
				raise EJvDBGrid.Create('Only TTable or derived classes or allowed for SortWith = swIndex');
    swFields:
      if not InheritsFromByName(DataSource.DataSet.ClassType, 'TCustomADODataSet') then
        raise EJvDBGrid.Create('Only TCustomADODataSet or derived classes or allowed for SortWith = swFields');
    swClient:
      if not InheritsFromByName(DataSource.DataSet.ClassType, 'TCustomClientDataSet') then
        raise EJvDBGrid.Create('Only TCustomClientDataSet or derived classes or allowed for SortWith = swClient');
    swUserFunc: ;
    swWhere: ;
  end;
  ...
  if Assigned(DataLink) and DataLink.Active and Assigned(FieldsToSort) and
    Assigned(DataSource.DataSet) and not (DataSource.DataSet.Eof and DataSource.DataSet.Bof) then
  begin
...
procedure TJvDBGrid.SetMultiColSort(const Value: Boolean);
begin
...
procedure TJvDBGrid.GetSortFieldName(var FieldName: string);
begin
  if Assigned(FOnGetSortFieldName) then
    FOnGetSortFieldName(Self, FieldName);
end;

procedure TJvDBGrid.SaveGridPosition;
begin
  FSavedBookmark := DataLink.DataSet.Bookmark;
  FSavedRowPos := DataLink.ActiveRecord;
end;

procedure TJvDBGrid.RestoreGridPosition(Mode: TResyncMode = [rmExact, rmCenter]);
begin
...
function TJvDBGrid.PrivateSearch(var ResultCol: Integer; var ResultField: TField;
  const CaseSensitive, WholeFieldOnly, Next: Boolean): Boolean;
var
	DSet: TDataSet;
...
function TJvDBGrid.Search(const ValueToSearch: Variant; var ResultCol: Integer;
	var ResultField: TField; const CaseSensitive, WholeFieldOnly, Focus: Boolean): Boolean;
begin
  Result := False;
...
function TJvDBGrid.SearchNext(var ResultCol: Integer; var ResultField: TField;
  const CaseSensitive, WholeFieldOnly, Focus: Boolean): Boolean;
begin
  Result := False;
...
procedure TJvDBGrid.DrawCellHighlight(const ARect: TRect;
...
var
	...
	FixedColsOffset: Integer;

	function IsMultiSelected: Boolean;
	begin
		Result:={FMultiSelect and }(SelCount > 0) and ((dgRowSelect in Options) or not (gdFocused in AState));
	end;
...
begin
	...
	Inc(ARow, TitleOffset); // ARow not used.

	FixedColsOffset:=FixedCols + IndicatorOffset;
	...
	begin
		...
		begin
			if (ACol >= FixedColsOffset + 1) and (ACol < ColCount - 1) then
				InflateRect(LRect, 4, 0)
			else if (ACol = FixedColsOffset) and (ACol = (ColCount - 1)) then
			begin
				// Keep LRect as it is.
			end
			else if ACol = FixedColsOffset then
				Inc(LRect.Right, 4)
...
{testing}
(*
		if Assigned(TInternalInplaceEdit(InplaceEditor)) and TInternalInplaceEdit(InplaceEditor).Visible then
		begin
			TInternalInplaceEdit(InplaceEditor).Brush.Style:=bsSolid;
			TInternalInplaceEdit(InplaceEditor).Repaint;
		end;
*)

		if (FHighlightColor = clHighlight) or (SelCount = 0) or IsMultiSelected then
	...
	begin
		...
		begin
			...
			if IsMultiSelected then
				Canvas.Brush.Color := FMultiSelectColor
			else
				Canvas.Brush.Color := FHighlightColor;
			...
			if IsMultiSelected then
			begin
				GradientFillCanvas(Canvas, GetShadowColor(FMultiSelectColor, 45),
					GetShadowColor(FMultiSelectColor, 10), LRect, gdVertical);
				Canvas.Font.Color := FMultiSelectFontColor;
				Canvas.Brush.Style := bsClear;
			end
			else
			begin
				GradientFillCanvas(Canvas, GetShadowColor(FHighlightColor, 45),
					GetShadowColor(FHighlightColor, 10), LRect, gdVertical);
				Canvas.Font.Color := FHighlightFontColor;
				Canvas.Brush.Style := bsClear;
			end;
		end
		else
...
procedure TJvDBGrid.SetEnabled(Value: Boolean);
begin
